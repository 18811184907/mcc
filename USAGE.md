...
Claude Code 侧：`/mcc:xxx`（打 `/mcc:` + Tab 可自动补全所有 MCC 命令）
...
| 命令 | 功能 | 产出位置 |
|---|---|---|
| `/mcc:prd` | 交互式 PRD 生成（7 phase Socratic 对话，问题驱动不编造需求） | `.claude/PRPs/prds/{slug}.prd.md` |
| `/mcc:plan` | 从 PRD 或描述生成自包含实施计划（含 mandatory reading、patterns to mirror） | `.claude/PRPs/plans/{slug}.plan.md` |
| `/mcc:implement` | 按 plan 执行，每步 5 级验证（type/lint/test/build/integration），失败立停 | `.claude/PRPs/reports/{plan-name}-report.md`，plan 归档到 `completed/` |
| `/mcc:pr` | 自动找 PR 模板、分析 commits、关联 PRP artifacts，push + create | 直接发 PR |

**典型流**：
```
/mcc:prd          → PRD
/mcc:plan         → Plan（含待读文件和代码模式）
/mcc:implement    → 执行 + 5 级验证
/mcc:pr           → 发 PR
```
...
| 命令 | 功能 |
|---|---|
| `/mcc:full-stack` | 9 步全栈特性流水线：需求 → DB 设计 → 架构 → DB/后端/前端实现 → test+sec+perf 并行验证 → 部署 → 文档。**2 个 user approval checkpoint** |
| `/mcc:full-review` | 5 阶段全面代码审查：质量+架构 / 安全+性能 / 测试+文档 / 最佳实践+CI/CD → 按 P0/P1/P2/P3 优先级汇总 |

### 🔍 审查（2 个）

| 命令 | 场景 |
|---|---|
| `/mcc:review` | **单点**审查：本地未提交 diff，或 `mcc:review 123` 审 PR #123（并行调 code-reviewer + security-reviewer） |
| `/mcc:full-review` | **全面**体检：模块级或项目级（见上面大流水线） |

### ✅ 验证 / 测试（4 个）

| 命令 | 场景 |
|---|---|
| `/mcc:verify` | 跑 verification-loop skill：build / type / lint / test / security / diff 6 阶段 |
| `/mcc:test-coverage` | 分析覆盖率，定位低覆盖文件，生成缺失测试到 80%+（委派 test-automator） |
| `/mcc:tdd` | 强制 RED → GREEN → REFACTOR，先写失败测试再实现 |
| `/mcc:e2e` | Playwright E2E 测试：生成 page object + 跑测 + 产出 artifacts |
...
| 命令 | 适合 |
|---|---|
| `/mcc:build-fix` | 构建/类型错误专项修复。3 次同错误自动停下问用户 |
| `/mcc:fix-bug` | 单个 bug 深挖：强制 root cause → 提 A/B 方案 → 用户确认再动手。禁止 retry / timeout 拉长。产出归档 `docs/mistakes/` |
| `/mcc:troubleshoot` | 多域诊断：bug / build / performance / deployment 四种快速判断 + 路由到上面两个专项 |

### 🧠 会话持久化（2 个）· 跨天必用

| 命令 | 场景 |
|---|---|
| `/mcc:session-save` | session 结束前写"已验证成果 / 失败路线 / 下一步 exact action"到 `~/.claude/session-data/YYYY-MM-DD-{shortid}-session.tmp` |
| `/mcc:session-resume` | 加载最近 session 文件，结构化 briefing 后等用户确认继续 |

### 📚 学习 / 沉淀（2 个）

| 命令 | 场景 |
|---|---|
| `/mcc:learn` | 从当前 session 提取可复用 pattern（错误解决 / 调试技巧 / 项目约定）→ `~/.claude/skills/learned/{pattern}.md` |
| `/mcc:skill-create` | 分析 git history（默认 200 commits）提取约定 + 文件共变 + 架构 + 测试模式 → 生成 SKILL.md |

### 🚪 入口（2 个）

| 命令 | 场景 |
|---|---|
| `/mcc:init` | 新项目首次入场：探测栈 + 提取约定 + 生成 `CLAUDE.md` |
| `/mcc:explain` | 用中文详细解释代码/函数/模块/概念（假设懂基础，补领域知识） |
...
| **verification-loop** | `/mcc:verify` 的实现体 |
...
| Mode | 触发 | 行为 |
|---|---|---|
| **brainstorming** | "I want to build" / "maybe" / "not sure" / "/mcc:prd" | Socratic 提问，不做假设，等用户确认 |
...
# Claude Code 里
/mcc:init           # 探测栈 + 生成 CLAUDE.md
/mcc:prd            # 第一个 feature 的 PRD
```
...
```
/mcc:prd            → PRD
/mcc:plan           → Plan（自包含，含 mandatory reading）
/mcc:implement      → 执行，每步 5 级验证
/mcc:review         → 单点审查
/mcc:pr             → 发 PR
```
...
跨天的话中间可以：
```
/mcc:session-save   # 下班前
/mcc:session-resume # 第二天
```

### 🌐 做一个新功能（复杂度高，全栈级）

```
/mcc:full-stack     # 9 步流水线，含 2 个 checkpoint
```
...
```
/mcc:verify         # 本地 6 阶段验证
/mcc:full-review    # 5 阶段 × 多维度审查
```

### 🐛 生产出错了

```
/mcc:troubleshoot "API 偶尔 500"  # 快速多域诊断
```

根据诊断路由：
- 如果是 build 问题 → 自动走 `/mcc:build-fix`
- 如果是单 bug → 自动走 `/mcc:fix-bug`
...
```
<project>/
├── CLAUDE.md                                # /mcc:init 生成（如没有）
├── AGENTS.md                                # Codex 侧（如装了 Codex）
├── docs/
│   ├── mistakes/bug-YYYY-MM-DD-{slug}.md   # /mcc:fix-bug 归档
│   └── adr/NNNN-*.md                        # architecture-decision-records 产出
└── .claude/
    └── PRPs/
        ├── prds/{slug}.prd.md               # /mcc:prd
        ├── plans/{slug}.plan.md             # /mcc:plan
        ├── plans/completed/                 # /mcc:implement 归档
        ├── reports/{plan}-report.md         # /mcc:implement 产出
        ├── reviews/
        │   ├── pr-{N}-review.md             # /mcc:review (PR mode)
        │   ├── local-{ts}.md                # /mcc:review (Local mode)
        │   └── full/{ts}/00-...05-final.md  # /mcc:full-review
        └── features/{slug}/01-...09-docs.md # /mcc:full-stack

<用户家目录>/.claude/
├── session-data/YYYY-MM-DD-{shortid}-session.tmp  # /mcc:session-save
└── skills/learned/{pattern}.md                     # /mcc:learn
```
...
A: 检查 3 点：
1. 装了没？`ls ~/.claude/commands/mcc/`（Claude Code 侧）
...
A: `cat ~/.claude/.mcc-hooks/hooks.json` 看 hooks 清单，`ls ~/.claude/commands/mcc/` 看命令，`ls ~/.claude/skills/` 看 skills（MCC 的装在同级，目录名能识别）。
...
用 Codex 时**不能**像 Claude Code 那样敲 `/mcc:xxx`（Codex 没有 slash command）。改成用"自然语言 + 引用 prompt 名"：
