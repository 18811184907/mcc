---
description: "多域问题诊断：bug / build / performance / deployment。快速定位→根因→修复。单 bug 深挖用 /mcc:fix-bug，专 build 错用 /mcc:build-fix。"
...
与 `/mcc:fix-bug` 的分工：troubleshoot 适合类型未知或跨域；fix-bug 适合已确认是 bug 且需要深度根因分析的单点问题。
...
| 问题类型 | 处理 |
|---|---|
| Build 类 | 转 `/mcc:build-fix` |
| Bug 类（单点深挖）| 转 `/mcc:fix-bug` |
...
### 代码 bug 调查
```
/mcc:troubleshoot "user service 里的空指针异常" --type bug --trace
# 系统性分析错误上下文和调用栈
# 识别根因 + 给出精准修复建议
```
...
### Build 失败分析
```
/mcc:troubleshoot "TypeScript 编译错误" --type build --fix
# 分析 build log 和 TS 配置
# 对常见编译问题自动应用安全修复（委派 /mcc:build-fix）
```

### 性能退化诊断
```
/mcc:troubleshoot "API response 时间变慢" --type performance
# 指标分析 + 瓶颈识别
# 给出优化建议和监控方向
```
...
### 部署问题
```
/mcc:troubleshoot "服务在 prod 起不来" --type deployment --trace
# 环境和配置分析
# 系统性验证部署前置条件和依赖
```
...
- 专 build 问题 → `/mcc:build-fix`
- 单 bug 深挖 → `/mcc:fix-bug`
- 修完验证 → `/mcc:verify`
- 复盘沉淀 → `/mcc:learn`（把解法变成 learned skill）
