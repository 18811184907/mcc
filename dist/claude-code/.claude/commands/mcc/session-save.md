---
description: "把当前 session 的已验证成果、失败路线、下一步 exact action 写入带日期的 session 文件，供 /mcc:session-resume 恢复。"
argument-hint: "(无参数)"
---
...
- 文件是只读历史——下一 session 通过 `/mcc:session-resume` 读，不改它
...
- 下一个 session：`/mcc:session-resume` 读最新文件
- session 中产出的可复用 pattern：`/mcc:learn` 转成 learned skill
