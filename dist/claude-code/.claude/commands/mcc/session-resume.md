...
开启一个新 session 时，通过读上次 `/mcc:session-save` 存的文件，把 Claude 的上下文完整恢复：已经做了什么、哪些方法已经试过且失败（关键）、下一步具体做什么。**不自动动手**——展示 briefing 等用户确认。
...
/mcc:session-resume 2024-01-15
  # 加载该日期最新的那份

/mcc:session-resume ~/.claude/session-data/2024-01-15-abc123de-session.tmp
  # 加载指定文件
```
...
3. 无文件时告诉用户：
   ...
   Run /mcc:session-save at the end of a session to create one.
   ```
   然后停。
...
**session 文件空或损坏**：
报告"Session file found but appears empty or unreadable. 需要用 /mcc:session-save 新建一份。"
...
- 恢复后，用户可能要 `/mcc:session-save` 在新 session 结尾再存一份新的

## 与其他命令的关系

- 上游：`/mcc:session-save`（产出文件）
- 并行：`/mcc:learn`（session 内学到的沉淀 skill）
- 加载完可直接接 `/mcc:implement` / `/mcc:tdd` / 其他具体工作命令
