...
### Skill 不可用时的降级（v2.1.2 加）

如果 `project-onboarding` skill 没装（用户用了 `--skip-skills` 装 MCC，或自己删了），不要直接卡死。**Claude 直接跑下面的精简 4 阶段**（不依赖 skill）：

```
Phase 1（30s）：5 路并行 Glob 直接扫
  Glob "package.json|pyproject.toml|Cargo.toml|go.mod|pom.xml|Gemfile|composer.json"
  Glob "**/main.{py,ts,js,go,rs}|**/__main__.py|**/index.{ts,js}"
  Glob "*.config.* | .env.example | Dockerfile | docker-compose*"
  Glob "tests/ | __tests__/ | test_*.py | *.test.* | *.spec.*"
  Glob "README* | docs/ | CHANGELOG*"

Phase 2（~1 min · 简化版，不派 agent）：
  Claude 自己读 Phase 1 找到的关键文件（最多 10 个），抽架构 / 入口 / 数据流

Phase 3（30s）：4 路 Grep
  Grep 命名模式（def / class / function / interface 抽样）
  Grep 错误处理（try/except 或 try/catch 出现频次）
  Grep 测试位置 + 风格
  git log --pretty='%s' -20 看 commit 风格

Phase 4（~1 min · 落盘）：
  写 .claude/PRPs/onboarding/{date}-onboard-report.md
  写 CLAUDE.md（≤100 行，wc -l 校验）
```

降级版总耗时 ~3 min（少了 agent 并行的深度，但有报告产出）。降级后告诉用户："本次为降级 onboard。装 `project-onboarding` skill 后重跑可拿更深产出。"
