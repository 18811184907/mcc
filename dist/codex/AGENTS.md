...
- **`mcc-backup-off`** — 关闭当前项目的备份（停止 post-commit 自动推送）。已推的历史保留不动。
- **`mcc-backup-setup`** — 首次给当前项目配置团队代码备份到公司仓库。一次性运行，之后用 /backup 日常同步。
- **`mcc-backup-status`** — 看当前项目的备份状态：远程位置、最近一次推送、待推送的 commit 数。
- **`mcc-backup`** — 一键备份当前项目到公司仓库：add + commit + push 一条龙。日常用。
...
- **`mcc-full-review`** — 5 阶段全面代码审查：质量+架构 / 安全+性能 / 测试+文档 / 最佳实践+CI/CD → 优先级汇总。模块或项目级；单点审查请用 /review。
...
- **`mcc-test-coverage`** — 分析测试覆盖率，定位低覆盖文件，生成缺失测试到 80%+。新功能请用 /tdd，E2E 请用 /e2e。
- **`mcc-troubleshoot`** — 多域问题诊断：bug / build / performance / deployment。快速定位→根因→修复。单 bug 深挖用 /fix-bug，专 build 错用 /build-fix。
...
收到 review 反馈时先验证、不表演同意、不盲目照做。反馈不清楚时强制先澄清再实施。

### requesting-code-review
派 code-reviewer subagent 审代码（用精确构造的上下文，不继承会话历史）。完成任务、合并前、卡壳时触发。

### subagent-driven-development
同会话内 dispatch 新鲜 subagent 逐 task 实现 + 每 task 两轮 review（spec + code quality）。执行已有 implementation plan 时自动触发。

### tdd-workflow
严格的 RED-GREEN-REFACTOR 工作流：先写失败测试、watch it fail、再写最小实现。实现任何 feature/bugfix 前自动触发；也是 /tdd 命令的 skill 实现。

### using-git-worktrees
建隔离 worktree + 自动选目录 + 安全校验。开始 feature 开发或执行 plan 之前触发，避免污染当前工作区。

### verification-loop
交付前 6 阶段验证（Build/Type/Lint/Test/Security/Diff）。/verify 命令触发。

### writing-skills
把 TDD 应用到写 skill 本身：先跑 baseline 压力场景、写最小 skill、再 refactor 堵漏洞。创建或编辑任何 skill 时触发。
