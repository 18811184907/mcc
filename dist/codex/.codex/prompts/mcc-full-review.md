---
description: "5 阶段全面代码审查：质量+架构 / 安全+性能 / 测试+文档 / 最佳实践+CI/CD → 优先级汇总。模块或项目级；单点审查请用 `mcc-review` prompt。"
...
与 `mcc-review` prompt 的区别：`mcc-review` prompt 是单点（本地未提交或单个 PR），本命令是全景。
...
- 单 PR / 单文件审查：`mcc-review` prompt 更合适
- 新造全栈特性：`mcc-full-stack` prompt 已包含审查 step
- 修 bug 后做 sanity check：`mcc-review` prompt 或 `mcc-verify` prompt
