...
**Related**: `mcc-troubleshoot` prompt（更轻量、多域分诊）, `mcc-verify` prompt（最终闸门）, `mcc-learn` prompt（把这个根因变成 learned skill）
