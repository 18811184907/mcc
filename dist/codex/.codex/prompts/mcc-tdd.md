...
**E2E Tests** → 用 `mcc-e2e` prompt 单独跑
...
1. 用 `mcc-plan` prompt 明确要造什么
2. 用 `mcc-tdd` prompt 按 RED-GREEN-REFACTOR 实现
3. 遇到 build error → `mcc-build-fix` prompt
4. 跑 `mcc-review` prompt 审查
5. `mcc-test-coverage` prompt 验证覆盖达标

---

## 与其他命令的关系

- 新功能：先 `mcc-plan` prompt，再 `mcc-tdd` prompt
- Bug fix：`mcc-fix-bug` prompt 的 Phase 4 会调用 TDD 流程
- 老代码补测试：`mcc-test-coverage` prompt
- 完成后：`mcc-review` prompt
