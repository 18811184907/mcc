...
```bash
# 这是真有内容的项目吗？空目录该用 /init
test -d src -o -d app -o -d lib -o -d packages || (
  echo "空目录用 /init 而不是 /onboard"
  exit 0
)

# 已有 CLAUDE.md 且最近 7 天内更新？
if [ -f CLAUDE.md ] && find CLAUDE.md -mtime -7 | grep -q .; then
  echo "已有近期 CLAUDE.md。要重新 onboard 吗？(y/N)"
  # 等用户确认
fi
