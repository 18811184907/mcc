...
运行 ``mcc-plan` prompt .claude/PRPs/prds/{name}.prd.md` → 自动选中下一个 pending phase 并生成实施计划。
```
...
- 之后：`mcc-plan` prompt 把每个 phase 转化为自包含实施计划
- 之后：`mcc-session-save` prompt 在 PRD 生成完保存 session，方便跨日继续
