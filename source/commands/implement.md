...
Run /mcc:plan <feature-description> to create a plan first.
```
...
> Next: `/mcc:review` 审查改动，或 `/mcc:pr` 直接发 PR。
```
...
- 上游：`/mcc:plan` 产出输入
- 之后：`/mcc:review` 审查，然后用 `git commit` 提交（自己写清楚的 commit message）
- 之后：`/mcc:pr` 创建 PR
- 若 PRD 还有下一个 phase：`/mcc:plan <prd-path>` 自动选下一 phase
