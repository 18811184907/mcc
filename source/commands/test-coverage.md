---
description: "分析测试覆盖率，定位低覆盖文件，生成缺失测试到 80%+。新功能请用 /mcc:tdd，E2E 请用 /mcc:e2e。"
...
分析既有代码的测试覆盖率，为低于 80% 的文件自动补测试到 80%+。**对象是已存在的实现**；新功能应先走 TDD（`/mcc:tdd`），这个命令补老债。
...
- 新功能：先 `/mcc:tdd`（RED-GREEN-REFACTOR），不要写完再补
- 用户流：`/mcc:e2e` 跑 Playwright
- 验证通过后：`/mcc:verify` 做总闸门
